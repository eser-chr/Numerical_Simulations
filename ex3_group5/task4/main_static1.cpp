...
int main(int argc, char *argv[]) try {
  ...
  // Print number of threads that are going to be used
  ...
  auto jacobi_iter = [N = opts.N](const auto &xold, auto &xnew,
    ...
    // all interior points
    size_t j, i; // Defining i and j before the parallelization so they can be private to each thread

    /* Section of parallelization */
    #pragma omp parallel private(i, j) shared(xold, xnew, h2)
    {
      #pragma omp for schedule(static, 1) // parallelized loop
      for (j = 1; j < N - 1; ++j) {
        for (i = 1; i < N - 1; ++i) {
          auto w = xold[(i - 1) + (j)*N];
          auto e = xold[(i + 1) + (j)*N];
  ...
  // Print total runtime of the computation
  std::cout << "Total runtime : " << duration.count() / 1e6 << " s" << std::endl;
